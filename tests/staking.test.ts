...
import { multiplier, solveMinimumBuild } from '../src/app/core/calculations';
import {
  permanentRefineRate,
...
test('daily Node boost time never reduces the stable minimum QN requirement', () => {
  ...
  const withTwoHours = solveMinimumBuild({
    targetGrindPerDay: 5_000,
    refineRate,
    vialHours: 2,
    rigs: [],
  ...
  assert.equal(withTwoHours.qns, withoutTemporaryBoost.qns);
  assert.equal(withTwoHours.productionFactorAtReady, 1);
});
